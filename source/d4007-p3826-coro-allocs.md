...
Coroutine-based I/O causes this gap to become acute. Asynchronous execution makes optimising away the coroutine frame allocation unlikely or impossible, and the impact of multiple extra allocations per I/O operation can be disastrous. The committee has explicitly placed this use case in scope (see Section 6.5); the standard must accommodate it.
...
### 2.1 Coroutines Are the User-Facing Model

The committee invested years in `co_await`, `co_yield`, `co_return`, and `promise_type` to give C++ a first-class facility for writing asynchronous code that reads like sequential code. SG4 mandated at Kona (November 2023) that networking be built exclusively on sender/receiver (see Section 6.5). [P3552R3](https://wg21.link/p3552r3) ("Add a Coroutine Task Type") exists so that users can write `co_await socket.async_read(buf)`. The task type is the coroutine integration layer for `std::execution`. If coroutines are not intended for I/O, the task type has no purpose.

For most C++ developers, coroutines represent the practical ceiling for writing asynchronous code. Composing raw sender algorithm chains requires understanding completion signatures, operation state lifetimes, and sender/receiver internals - expert-level template metaprogramming that the typical developer writing HTTP route handlers, database queries, or file I/O cannot reasonably be expected to perform. Those developers can write task-returning functions that perform `co_await`. That is the value proposition of the coroutine integration.
...
// at this rate, allocation strategy becomes a throughput concern
```
...
Modern general-purpose allocators (jemalloc, mimalloc, tcmalloc) achieve impressive throughput via per-thread caching. They reduce contention on the global heap and handle the allocation patterns that I/O servers produce. However, coroutine frames are short-lived, uniformly sized, and frequently allocated on one thread but freed on another - a pattern that general-purpose allocators handle but do not specifically optimize for.

A recycling allocator tuned for coroutine frames maintains a freelist of previously used blocks in the same size class. The hot path is a pointer swap - no search, no coalescing, no bookkeeping. The allocator does not allocate and free at all in the conventional sense; it recycles. No general-purpose allocator can match this, because it must handle arbitrary sizes, lifetimes, and alignment without domain knowledge.
...
With proper allocator support, coroutine-based networking matches or exceeds callback-based frameworks in throughput (see Section 6.5). Stateful allocators are not a luxury; for coroutine-based I/O they are a prerequisite for competitive performance.

### 3.2 HALO Cannot Help

HALO (Heap Allocation eLision Optimisation) allows compilers to elide coroutine frame allocation when the frame's lifetime is provably bounded by its caller. For I/O coroutines launched onto an execution context, the frame outlives its caller - the caller returns immediately after launching the operation, and the frame persists until the OS signals completion.
<!-- JMB: This seems to be true if the calling code is *not* a coroutine that will be resumed when the i/o operation completes. -->
<!-- JSL: @Josh, This what? Please make it clear what's true when. @Mungo, Also, what's the approximate relative frequence of the two and under what circomstances? JSL -->
<!-- MG: Not sure I understand what Josh is saying here. --> The compiler cannot prove bounded lifetime when the lifetime depends on an external event; hence, HALO cannot apply in this common case, and allocation is mandatory. See Appendix A.1 for a deeper analysis and code example.
...
<!-- DONE: JMB: It's also worth noting that HALO does nothing when the coroutine's implementation is not visible. -->
<!-- DONE: JSL: Absolso-F#$%ing-lutely say that.  OMFG!!! JSL -->
<!-- JSL: Yes I realize that lots of this stuff is templates.  Can we please try to gustimate how often it might be that the co-routines impl. is not vissable?  What criteria affects that decision, etc. JSL -->
...
[P3552R3](https://wg21.link/p3552r3) ("Add a Coroutine Task Type") introduces a two-tier model for execution resources in sender/receiver:
...
2. **Silent performance degradation.** Omitting `allocator_arg` from one call does not produce a compile error - the child silently falls back to the default heap allocator. The program runs correctly, but under production load the resulting allocation overhead can degrade throughput (see Section 3.1 benchmarks). The degradation surfaces only under load, making it difficult to detect during development.
...
The standard library partially addresses a similar problem for containers through *uses_allocator* construction and `scoped_allocator_adaptor` - though `scoped_allocator_adaptor` is widely considered one of the most complex parts of the standard library, and custom aggregates must still propagate allocators manually. Automatic allocator propagation has been a long-standing difficulty in C++, and coroutines are not the first context where it has proven hard.

However, sender/receiver introduces a constraint that containers do not face: `operator new` runs before `connect()`, so the receiver's allocator is structurally unreachable at the point of frame allocation. The difficulty being inherent to C++ does not argue for freezing an interface that adds a new dimension of that difficulty.

It is worth distinguishing two separable concerns:

- **The sequencing gap** (specific to sender/receiver): the receiver's allocator cannot reach `operator new` because   the frame is allocated before the receiver exists. The   deferral argument in this paper rests primarily on this   constraint.
- **Nested propagation** (structural to C++ coroutines): allocators do not propagate automatically through nested   coroutine calls. This difficulty exists in any coroutine   framework, including the authors' own work. Library-level   solutions to this problem exist (see Section 4.6).
...
<!-- JSL: The comments above are one two close to the longest statement.  Standard is two spaces if possible.  Also, I'd like to see consistent punctuation:
1. Colon after first line.
2. Capitalization and Period association honored consistently JSL -->
...
[P2300R4](https://wg21.link/p2300r4) ("std::execution," 2022-01-18) acknowledges this sequencing:
...
### 4.6 An Existence Proof

The authors do not propose that the standard must adopt any specific mechanism for allocator propagation. The purpose of this section is narrower: to demonstrate that at least one library-level solution exists, that there may be others, and that the design space merits exploration before the ABI is frozen.

[P4003R0](https://wg21.link/p4003r0) demonstrates a working solution to both the sequencing gap and automatic propagation through nested coroutine calls. The mechanism has three parts.

**Two-phase invocation.** The launch site uses a double-call syntax that relies on C++17 postfix evaluation order ([expr.call]):

```cpp
// allocator supplied once at the launch site
run_async(ex, alloc)(my_task());

// within a coroutine, same pattern
co_await run(alloc)(subtask());
```

The postfix-expression is sequenced before the argument expression. The first call returns a wrapper whose constructor stores a `pmr::memory_resource*` in thread-local storage. The second call evaluates the coroutine expression; `operator new` reads TLS and allocates the frame from the chosen resource. The single-call form `run_async(ex, my_task())` is rejected because argument evaluation order would place frame allocation before the launcher can set TLS - a correctness constraint, not a style preference.

**PMR type erasure.** Typed allocators are stored as `pmr::memory_resource*`, giving a uniform interface for all downstream coroutines. US 255 notes that "the used allocator type needs to be known when the coroutine is created." With PMR, the type is always `pmr::memory_resource*`; only the runtime pointer varies. Virtual dispatch cost is negligible relative to the allocation itself.

**Propagation via promise restore.** On every `co_await` resume, `await_resume` restores TLS from the allocator stored in the coroutine's heap-stable environment (`io_env`). Child coroutines inherit the allocator automatically:

```cpp
task<void> handle_upload(route_params& rp)
{
...
No allocator appears in the signature. No forwarding at call sites. Every child coroutine's `operator new` reads the allocator from TLS, which the parent's `await_resume` just restored.

**Thread-local safety.** Thread-local storage has a well-deserved reputation for hidden coupling. The concern deserves a direct answer:

- TLS here is a **write-through cache**, not a source of truth. The canonical allocator lives in `io_env`, a   heap-stable structure in the launch coroutine's frame.   Every resume point restores TLS from `io_env` before the   coroutine body continues.
- TLS is read in exactly one place: `promise_type::operator new`. No algorithm changes   behavior based on TLS contents.
- Thread migration: when a coroutine suspends on thread A and resumes on thread B, `await_resume` *writes* TLS on   thread B before the body continues. TLS is never read on   a thread unless the coroutine that wrote it is actively   executing on that thread.
- Deallocation is thread-independent. Each frame stores its `memory_resource*` in a footer appended to the allocation.   `operator delete` reads the footer, not TLS. A frame   allocated on thread A can be freed on thread C.
- Precedent: `std::pmr::get_default_resource()` is a global (optionally thread-local) allocator propagation mechanism   accepted in C++17. The approach described here applies the   same principle with tighter scoping - per-chain rather   than per-process.

The design space may contain other approaches: a new coroutine customization point that fires before the child's `operator new`, deferred allocation in `task<T>` until `connect()`, or mechanisms not yet considered. The point is not that this specific solution is the right one, but that solutions exist and deserve exploration time before the ABI is frozen.
...
### 6.4 Shipping Without `task`

A natural compromise is to ship `std::execution` in C++26 without `task` and iterate on the coroutine integration for C++29. This would preserve the sender/receiver protocol, avoid ABI lock-in on the coroutine integration, and keep the CPU/GPU use cases on track.

The tradeoffs deserve honest assessment. `task` is the primary usability layer for `std::execution`. Without it, users must write raw sender pipelines for coroutine code. SG4 mandated networking on sender/receiver (see Section 6.5); the task type is what makes that usable via coroutines. [P3552R3](https://wg21.link/p3552r3) was forwarded to LWG with a recommendation to apply for C++26 "if possible" - removing it would leave the framework without its coroutine integration during the period when networking is expected to build upon it.

Additionally, the allocator gap exists in `connect()` itself (LWG 4356), not only in `task`. Shipping without `task` does not fully resolve the architectural concern described in Section 4.

The authors consider this option worth the committee's discussion and do not dismiss it. Whether the costs outweigh the benefits is a judgment the committee is best placed to make.
...
The same paper reports a 48% throughput improvement when using a recycling frame allocator instead of optimized third party allocators such as mimalloc, and a whopping 202% improvement over `std::allocator` on a 4-deep coroutine call chain (2 million iterations). Without allocator support, those gains disappear.
...
[P2300R4](https://wg21.link/p2300r4) (2022-01-18) acknowledges this sequencing:
...
By contrast, [P4003R0](https://wg21.link/p4003r0) demonstrates a model where the allocator propagates automatically through the coroutine call chain. The allocator is set once at the launch site; every child coroutine's `operator new` reads it from thread-local storage without any parameter forwarding:
