...
Six papers: two competing rule systems, one predictive scoring model, a thirteen-term vocabulary of committee failure modes, and the case for a missing integer type. Five papers trace the structural distance between the committee the ISO Directives chartered and the committee SD-4 built. One makes the evidence case for `std::int128_t`.
