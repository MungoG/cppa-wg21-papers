...
P3100R8's Section 2, "History and polls", lists what it presents as the complete committee history. P3100R8, dated July 2026, is the latest revision at the time of writing; the classification in Table 1 applies unchanged to any polls a later revision adds. There is no independent public list to check it against. Table 1 reproduces every poll from that section, quoted from the paper, with the tallies the paper itself gives.

Table 1: All six polls in P3100R8's self-reported history (its Section 2). Poll text abridged only by ellipsis; tallies and result labels as printed in P3100R8. The rightmost column classifies what each poll asked for.
...
The Hagenberg poll and its tally are independently public in [P3656R1](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2025/p3656r1.pdf)<sup>[10]</sup>, the white paper process document by its appointed editors. The remaining tallies are as self-reported in P3100R8.
...
The accumulation works in plain steps. Each recorded consensus becomes the starting point for the next question, and each one raises the cost of objecting later. That is what this paper means by a ratchet: not a claim that the outcome is inevitable, but a series of small forward steps that are individually easy and collectively hard to undo. Case-by-case wording review completes the pattern. The Croydon poll routes the proposal into review of 77 cases, one at a time. Each case review is a small, technical, reasonable question, and none of them is the architecture question of P3100R8's Section 4.4. When the last case is approved, the architecture will have been settled without ever being polled on its own. Its one ballot appearance was as "a good basis" for the abandoned white paper (Table 1, poll 4).
...
- **It decides who writes guarantees.** Under P3984R0's model, a profile writes the guarantee itself: "A profile cannot change the semantics of a program beyond defining the meaning of some forms of undefined behavior", and its overflow example has the profile choose wraparound, saturation, or an exception<sup>[6]</sup>. Under P3100R8's Section 4.4, a profile defines nothing; it selects a configuration of the proposal's semantics. The difference is ownership: the profile author writes the guarantee, or the preset author chooses from the proposal's menu.
...
The claim of this section is deliberately narrow. Using the search method below, this paper found no published WG21 paper, through the 2026-05 pre-Brno mailing (the most recent published mailing at the 2026-07-07 fetch), that directly contests P3100R8's characterization of Profiles. This is a statement about the public paper record only. It says nothing about whether anyone has objected in committee discussion, which this paper does not examine.
...
[35] [P4306R0](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2026/p4306r0.pdf) - "Configuring Runtime Checking: Profiles and Implicit Contract Assertions" (Vinnie Falco, Ville Voutilainen, 2026).

[36] [P4310R0](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2026/p4310r0.pdf) - "Hasta la Vista, Undefined Behavior: Why Implicit Contract Violations Should Terminate" (Vinnie Falco, 2026).
