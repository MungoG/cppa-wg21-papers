...
A proposal whose authors state that every existing implementation of C++ already conforms to its wording is deciding which safety framework governs C++.
...
This paper proposes no wording and makes no technical request. It recommends only that WG21 decide the architecture question explicitly.
