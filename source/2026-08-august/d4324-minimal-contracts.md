...
Section 2 motivates why constification and exception propagation should be per-assertion policies rather than fixed language rules. The contributions are:

1. A minimal core-language contracts design: two syntactic jobs, a three-step compiler algorithm, and a library-owned response, specified at design granularity (Section 3).
2. A statement of how the design relates to the C++26 working draft, and an account of what changing the draft costs at the current stage (Section 4).
3. An architecture in which contracts and profiles are independent peers, so that no core-language undefined behavior is routed through the contract-violation handler (Section 5).
4. A reference-implementation plan built on existing compiler forks (Section 6).
...
P2900 fixes constification and exception-to-violation conversion as language rules applied to every predicate. This section shows in code what each costs, and why the design makes them per-assertion policies with a constification default that reports the predicates whose meaning it would change rather than choosing for the programmer.
...
The design makes constification a per-assertion `constification` policy (Section 3.2) whose default is `diagnose`: the compiler reports a predicate whose meaning would change under constification and makes the author choose `on` or `off`, and says nothing when the meaning would not change, which is every predicate over const-correct operands. Consistency is the outcome in the common case, and a decision is forced only where constification actually matters. Whether to prefer constification or consistency when they differ is an engineering judgment, and the design declines to make it silently on the programmer's behalf.
...
Here constification is the feature working as intended: it rejects a predicate that would mutate the container, a protection P3071R1<sup>[23]</sup> was written to provide. The design keeps that protection available rather than removing it, which is why constification is a policy a control object selects and not a behavior the language drops. The `diagnose` default surfaces this case too, because the constified and unconstified predicates differ, and the author decides whether to allow it.

Naming the policy in an enumeration keeps every choice explicit and greppable, and lets the standard add values from field experience, the way it has added evaluation semantics. It also keeps the question out of the "remove it or keep it" frame the committee has already settled: at Hagenberg, EWG declined to remove constification (Section 4). This design does not remove constification; it relocates the choice into a per-assertion policy and, by default, reports the case where the choice matters.
...
The syntax `pre`, `post`, and `contract_assert` does exactly two things. It binds a predicate to a declaration or a statement, and it names an assertion-control object that governs the predicate. The predicate is an operand of the introducer, not an argument passed to a function. The compiler controls whether and when it is evaluated, so an ignored assertion never evaluates it (Section 3.8); a function call, by contrast, evaluates its arguments before the call runs.
...
1. If T::is_ignored(cfg) is true:
       ...
       Stop.
2. Select on T::constify: for off, evaluate the predicate as written; for on, evaluate the
       constified predicate; for diagnose, resolve the predicate with and without constification
       and, if the two would call different functions, diagnose; otherwise evaluate as written.
...
The `diagnose` value of `constify` names a check the design is actively exploring rather than one the reference implementation yet provides. To detect divergence the compiler performs overload resolution on the predicate twice, once with constification and once as written, and compares the entity selected at each call, operator, and conversion; a predicate diverges when any node selects a different entity, or is well-formed one way and ill-formed the other. On divergence the assertion is diagnosed, and the author resolves it by naming `constification::on` or `constification::off`, or by opting a subexpression out with the `mutable` or `noconst` mechanisms P3592R0<sup>[8]</sup> proposes. A predicate over const-correct operands does not diverge, so the two resolutions coincide and no choice arises. This detection is compile-time work beyond reading a member; its specification and a prototype are future work (Section 6).
...
enum class constification { off, on, diagnose };
...
concept assertion_control =
  ...
  requires (T c, const char* comment, std::source_location loc, evaluation_config cfg) {
    ...
    { T::constify             } -> std::same_as<constification>;     // step 2
...
struct default_control {
  ...
  static constexpr constification constify = constification::diagnose;
...
The `constification` enumeration extends differently. The compiler reads and acts on it to steer code generation rather than forwarding it to a library, so its values must be ones the compiler implements. It grows by standard revision, for example a warning-only variant of `diagnose`, and not through the open vendor range `evaluation_config` allows, because a `constify` value the compiler did not recognize would have no defined code generation.
...
struct review {
  ...
  static constexpr constification constify = constification::on;
  static constexpr bool assumable = false;
...
struct mandatory {
  static constexpr bool is_ignored(evaluation_config) { return false; }
  static constexpr constification constify = constification::off;
...
Two differences are substantive. First, constification is a property of the control object rather than a language rule: `constify` is a three-valued `constification` policy, and `default_control` sets it to `diagnose`, so a predicate means what the same expression means in the function body unless the two would bind different functions, in which case the assertion is diagnosed and the author names `on` or `off`. Second, an exception that escapes a predicate propagates as an ordinary exception, stopped at a `noexcept` boundary as any exception is, rather than becoming a contract violation. Both behaviors remain available as control-object properties; they stop being imposed by the language.

Both behaviors are separable, and the separation has implementation experience. P4005R0<sup>[2]</sup> reports "implementation experience with a mix of contract assertions with 'constification' and without" and, likewise, with and without converting an escaping predicate exception into a violation. Berne's P3261R0<sup>[7]</sup> enumerates removing constification as "Proposal 1: No const-ification," and P3592R0<sup>[8]</sup> proposes a `mutable` specifier "to simply turn off const-ification entirely within a contract assertion predicate." The design here makes `diagnose` the default, with `on` (constify) and `off` (consistency) the explicit choices a control object names.
...
The `diagnose` constification policy (Section 3.2) is the one element not yet prototyped. It requires the compiler to resolve a predicate both with and without constification and compare the results, which is more than reading a member off the control object. The design is exploring it now, and a later revision will report a prototype on the same fork; until then this paper presents `diagnose` as a proposed default rather than an implemented one.
...
| Verifiable by local inspection | | Yes | Under constification a predicate can bind a different overload than the same text in the body, so "overload resolution might quietly invoke different functions in the two contexts"<sup>[8]</sup>; the minimal core defaults to diagnosing that divergence and otherwise evaluates the predicate under the usual rules<sup>[2]</sup>. Build-time semantic selection remains in both, so the difference is the meaning of the expression, not the selected semantic. |
...
The scoring for the minimal core is the scoring of its default control object. A control object that sets `constify` to `on` reintroduces the local-inspection cost for the assertions that name it, and one that converts an escaping exception into a violation reintroduces the exception-handling scaffolding, so those rows would read for such an object as they do for P2900. The difference the table records is that P2900 applies the two behaviors unconditionally, while here they are opt-in per assertion and off by default.
...
concept assertion_control =
  ...
  requires (T c, const char* comment, std::source_location loc, evaluation_config cfg) {
    ...
    { T::constify             } -> std::same_as<constification>;
...
struct default_control {
  static constexpr bool is_ignored(evaluation_config cfg) { return cfg == evaluation_config::ignore; }
  static constexpr constification constify = constification::diagnose;   // report divergence, then choose
  static constexpr bool assumable = false;
  violation_response operator()(const char* comment, std::source_location loc,
                                evaluation_config cfg) const;
};
