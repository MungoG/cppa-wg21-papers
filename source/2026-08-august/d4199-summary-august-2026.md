---
title: "A Reader's Guide to the August 2026 Mailing"
document: D4199R0
date: 2026-08-31
intent: info
audience: LEWG
...
Eleven papers reconstruct how Contracts reached C++26 without a dedicated architecture ballot, propose five governance correctives lifted verbatim from ISO Directives, and deliver a complete profiles specification covering seventy-seven cases of core-language undefined behavior - backed by deployment evidence from eight shipping systems - that requires zero foundational wording changes to the standard.

This paper summarizes 11 papers published in the
...
A single profile attribute, zero foundational wording changes, and seventy-seven cases of core-language undefined behavior covered by a specification whose terminating response already ships across eight production systems - that is what P4317R1 delivers under the P3589R2 Profiles framework. P4297R1 reconstructs the seven consensus polls that advanced P3100 and shows that not one of them adopted the architecture placing Profiles beneath its machinery; P4306R1 assembles more than a hundred cited sources - vendor documentation, maintainer statements, and deployment reports from Google, Apple, Mozilla, Android, and Chrome - and measures both candidate owners against four criteria already in the committee record; P4330R0 walks ten contracts papers line by line and catalogues the mechanisms by which a profiles safety framework loses its independence. Together these four papers establish that the alternative to waiting for contracts already exists, already works, and already ships - libc++ hardening measured at 0.30 percent overhead across hundreds of millions of lines at Google.

The compound finding across this cluster is architectural: the contracts substrate captures the violation response, the configuration design space, and the definition of class invariants - leaving profiles with no independent path to deployment. Each paper documents a different axis of that capture, and reading them together makes visible what no single paper shows: the foreclosure is not one decision but a distributed accumulation of sixty-three incremental merges, each individually plausible, whose aggregate effect was never the subject of a standalone ballot.
...
### 3.3. P4240R0 - Did ISO or SD-4 Govern the P2900 Ballot?

Nineteen of twenty-six National Bodies filed ballot comments on P2900 Contracts, and the twenty-two-author defense was finalized five days after the ballot closed - while the technical dispute on the SG15 list was still live. P4240R0 applies two rule sets to the same NB comment phase and documents what each produces: the ISO/IEC Directives require reconciliation of conflicting arguments before consensus can be declared, while SD-4 provides a mechanism chain - bird-in-hand priority, prior-consideration reclassification, consensus ratchet, credibility cost - that makes reversal procedurally near-impossible from inside. The paper traces the exact dates (consensus declared February 2025, objections arriving October 2025, defense assembled October 6) and shows that SD-4 was presented to ballot participants as if it operated at the SC-level ballot where it carries no ISO procedural standing. The comparison is systematic, the timeline is sourced to public archives, and the conclusion is structural: one system asks the proposal author to reconcile conflicting arguments, the other asks the objector for a paper number.

### 3.4. P4297R1 - Severing P3100's Profiles Claim from Its Case-by-Case Review
...
### 3.5. P4302R1 - Any Delegate May Object to a Poll on an Unmailed Revision
...
### 3.6. P4306R1 - Configuring Runtime Checking: Profiles and Implicit Contract Assertions
...
### 3.7. P4308R1 - Eight Responses to a Throwing Implicit Contract Assertion
...
### 3.8. P4310R1 - Hasta la Vista, Undefined Behavior: Why std::core_ub Should Terminate by Default
...
### 3.9. P4317R1 - A Profile for Runtime-Checkable Core-Language Undefined Behavior: std::core_ub

Seventy-seven cases of core-language undefined behavior, guarded by a single profile attribute and zero foundational wording changes to the standard - where the alternative routing requires six. P4317R1 specifies std::core_ub under the P3589R2 Profiles framework, covering every runtime-checkable case enumerated by Doumler and Berne in P3100R8, with a terminating response whose form already ships across eight production systems (libc++ hardening measured at 0.30 percent overhead across hundreds of millions of lines at Google). The paper scores the profile against all twelve SD-10 and D&E principles, compares three candidate violation responses drawn from deployed systems (Apple trap, libstdc++ diagnostic-then-abort, Bloomberg non-returning handler), and addresses seven potential concerns including the Hagenberg precedent and the sanitizer-cost gap for the 58 instrumented cases. It requests no poll and proposes no wording - it asks whether the form production already runs is worth standardizing for C++29.

### 3.10. P4318R1 - Transient Benefit, Perpetual Cost: Implicit Core-Language Assertions
...
### 3.11. P4330R0 - Analysis of Contracts Papers
...
This reading guide covers 11 papers from the August 2026 mailing.
...
[3] P4240R0 - "Did ISO or SD-4 Govern the P2900 Ballot?" (Vinnie Falco, 2026).

[4] P4297R1 - "Severing P3100's Profiles Claim from Its Case-by-Case Review" (Vinnie Falco, 2026).

[5] P4302R1 - "Any Delegate May Object to a Poll on an Unmailed Revision" (Vinnie Falco, 2026).

[6] P4306R1 - "Configuring Runtime Checking: Profiles and Implicit Contract Assertions" (Vinnie Falco, 2026).

[7] P4308R1 - "Eight Responses to a Throwing Implicit Contract Assertion" (Vinnie Falco, 2026).

[8] P4310R1 - "Hasta la Vista, Undefined Behavior: Why std::core_ub Should Terminate by Default" (Vinnie Falco, 2026).

[9] P4317R1 - "A Profile for Runtime-Checkable Core-Language Undefined Behavior: std::core_ub" (Vinnie Falco, 2026).

[10] P4318R1 - "Transient Benefit, Perpetual Cost: Implicit Core-Language Assertions" (Vinnie Falco, 2026).

[11] P4330R0 - "Analysis of Contracts Papers" (Vinnie Falco, 2026).
