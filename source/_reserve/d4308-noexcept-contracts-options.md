...
This paper makes four contributions. First, it enumerates eight responses to a throwing implicit contract assertion: Option 0, the four P3100 options A through D, and three further options, E, F, and G. Second, it restores Option 0, the option P3100's premise forecloses, and states both its case and the record that set it aside. Third, it proposes Options E, F, and G. Fourth, it compares all eight against one standard, six requirements drawn from P3100R8's prose together with five dimensions from the public record.
...
One part of the premise is under-evidenced. The magnitude of the breaking change is unmeasured: the public record contains no study of how much code depends on the `noexcept` of an expression P3100 would make checkable, in either direction. So the premise rests on the principle and the poll, which are on the record, and on a breaking-change magnitude that is not. The same standard - name the evidence, mark the gap - governs every deployment claim in Section 7.
