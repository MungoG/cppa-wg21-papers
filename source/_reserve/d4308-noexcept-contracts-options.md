...
The response to a throwing implicit contract assertion is being decided among four options; there are eight.

C++26 shipped Contracts, and P3100 extends them with implicit assertions that check core-language undefined behavior on the C++29 track. When such a check's violation handler throws, an exception can leave an expression the `noexcept` operator reports as non-throwing, and P3100 resolves this by holding the operator's value fixed, a premise that narrows the response to the four options A through D. This paper restores the option that premise forecloses - Option 0, the value-reporting operator - and adds three the author proposes, E, F, and G, then compares all eight against six requirements drawn from the P3100 proposal and five dimensions the public record adds: deployment, security, compatibility direction, diagnostics, and implementation experience. The comparison finds the space larger than the four before EWG, and finds the narrowing premise supported by a design principle and a poll but unmeasured as a breaking change. This paper makes no request.
...
The options labeled A through D are presented from their published source, [P3100R8](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2026/p3100r8.pdf)<sup>[1]</sup>, "A framework for systematically addressing undefined behaviour in the C++ Standard" (Timur Doumler and Joshua Berne). The author is characterizing that published position, does not speak for its authors, and has written the A-D material to a standard its authors could endorse. Option 0 and options E through G are the author's own contribution.

One limitation of this paper's method is that its comparison rests on a set of six requirements derived from P3100R8's own prose. A different requirement set could order the options differently. Section 7 discloses the set, names its provenance, and applies it identically to every option.

This paper is one of a series in the same mailing. Its companion, [P4306R0](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2026/p4306r0.pdf)<sup>[2]</sup>, "Configuring Runtime Checking: Profiles and Implicit Contract Assertions" (Vinnie Falco and Ville Voutilainen), carries the deeper configuration, dialect, and Profiles analysis. This paper scopes to the response-option space and cross-references P4306R0 rather than repeating it.
...
This section introduces the paper's contributions and assumptions, then defines the terms the rest of it uses and states the premise every option shares.

This paper makes four contributions. First, it enumerates the full response space for a throwing implicit contract assertion: Option 0, the four P3100 options A through D, and three further options, E, F, and G. Second, it restores Option 0, the option P3100's premise forecloses, and states both its case and the record that set it aside. Third, it proposes Options E, F, and G. Fourth, it compares all eight against one standard, six requirements drawn from P3100R8's prose together with five dimensions from the public record.

The paper assumes the premise P3100 adopts, that an implicit contract assertion must not change the value of the `noexcept` operator (Section 2.3), and it takes the C++26 Contracts facility as given. It is scoped to implicit contract assertions on core-language expressions; explicit contract assertions, and the general question of whether C++29 should add non-throwing evaluation semantics, are out of scope except where an option touches them.

A reader fluent in C++26 Contracts can skip to the premise in Section 2.3; the definitions come first because the options differ only in details these terms make precise. Section 2.1 describes the P3100 model and the evaluation semantics, Section 2.2 describes the `noexcept` operator and the interaction that creates the decision, and Section 2.3 states the shared premise and the two questions the discussion conflates.

### 2.1. The P3100 model and the evaluation semantics
...
### 2.2. The noexcept operator and the interaction it creates
...
### 2.3. The shared premise and the two questions
...
The response space is Option 0 together with Options A through G, and the two tables below preview all eight before Sections 4 through 6 define them in full. Table 1 scores each option against six requirements drawn from P3100R8's prose; Table 2 gives each option's response shape and points to its deployment record.
...
### 4.2. Option A: the exception propagates
...
SG21's recorded preference is for Option A. P3100R8 states it plainly: the options "were first proposed in [P3541R1]. This paper was discussed in SG21 and resulted in strong consensus in favour of Option A and against Option B because the latter precludes the stack-unwinding approach to handling implicit contract violations."<sup>[1]</sup>
...
Option C's cost is the number of semantics: adding observe-noexcept and enforce-noexcept alongside observe and enforce doubles the checking semantics a programmer and an implementation reason about, and D4298R0 marks the enumerator values themselves as provisional.

Option D provides three properties: it gives implicit assertions the unchanged `noexcept` meaning and exception model of Option B, because they never reach a throwing handler; it keeps the new non-throwing semantics available for explicit assertions, unlike Option B's blanket rule; and it forecloses the escape path for implicit assertions by construction. D's own text places it "essentially the same as Option B" for implicit assertions, with the added ability to use the non-throwing semantics for explicit assertions as well, at the cost of removing the escape path Option A preserves.<sup>[1]</sup>

Options A through D thus span the response space P3100 mapped: A lets the exception escape and keeps every semantic, B forbids escape and keeps the exception model unchanged, C offers both by adding semantics, and D offers the non-throwing side of C for implicit assertions. SG21's recorded consensus is for A. What each option looks like when the space is read against deployment, security, and compatibility is the subject of Section 7.

---

## 5. Option 0

Section 2.3 named the option foreclosed by the premise: letting the `noexcept` operator return `false` for an expression that carries a potentially-throwing implicit assertion. This section restores that option - Option 0 - and gives it the treatment every other option gets: the case for it at full strength, then the costs that removed it from the enumeration. Section 5.1 makes the case, Section 5.2 states the costs that end it, and Section 5.3 states the question the analysis leaves for Section 6.

### 5.1. The case for a value-reporting operator

Option 0 is the reading of `noexcept` that P3541R1 and P2969R0 describe as the natural one. P3541R1 states it as its Option 1: "Any construct that might end up in either undefined behavior or contract violation is potentially-throwing and this is reflected by the noexcept-operator."<sup>[6]</sup> P2969R0 reaches the same place from the other direction: "since contract checks can throw an exception, we should treat them as potentially-throwing in the language," which "would mean that the noexcept operator ... would return false," and the paper calls this treatment "straightforward and intuitive."<sup>[9]</sup> This much should be granted without reservation: for an expression that can throw, an operator returning `false` reports the potentially-throwing value, and an operator returning `true` does not.
...
### 5.2. Why the value-reporting operator does not survive
...
In the `ignore` or `assume` translation unit, `h` has type `void() noexcept` and one mangled name; in the throwing-`observe` translation unit, `h` has type `void()` and a different mangled name. The two object files do not link, or link with mismatched declarations of one entity, which is an ODR violation. This is not a new failure mode: making `noexcept` part of the type in C++17 changed mangled names, which is why GCC ships `-Wnoexcept-type` to "warn if the C++17 feature making noexcept part of a function type changes the mangled name of a symbol relative to C++14."<sup>[15]</sup> The build-mode-dependent operator re-triggers that episode, keyed to the contract build mode instead of the language version.

Second, the unconditional form avoids the link break only by making the operator return `false` for such an expression in every build, whether or not this build can throw. That escapes the ODR problem by paying the source-level price Section 2.3 describes: `noexcept(x + 1)`, `noexcept(*p)`, and `noexcept(a[i])` flip from `true` to `false` for nearly every scalar and pointer expression in the language, the mass source-level change Section 2.3 described. It also runs against the C++26 design principle directly: P2900R14 Principle 3 states that "the mere presence of a contract assertion ... should not change ... the value returned by the noexcept operator,"<sup>[3]</sup> and the unconditional form is that change. P2834R1 reached the general form of this conclusion for build modes, finding that allowing such changes in program behavior across contract-checking build modes "is actively harmful."<sup>[16]</sup>

Both forms were before SG21, and SG21 recorded a decision. It polled making the operator return `false` for such expressions and reached consensus against, SF/F/N/A/SA of 0/2/0/7/9, while it polled keeping the operator's answer `true` and reached consensus for it, 5/10/1/2/1.<sup>[7]</sup>
...
### 5.3. The question the analysis leaves open

The value-reporting way to let an implicit assertion throw fails on both forms: the build-mode-dependent operator breaks linking, the unconditional operator is the mass source-level change Section 2.3 describes and the principle forbids, and SG21 polled the option down. The value-preserving way, Option A, remains available, and reaches an escaping exception by holding the operator's value while shifting its meaning (Section 4.2). With the value-reporting operator removed, the question the enumeration raises is no longer how to report a throwing implicit assertion, but whether an implicit assertion should invoke a throwing handler at all. Options E, F, and G answer that question, and Section 6 presents them.

---

## 6. Additional Options (E, F, G)

Sections 4 and 5 leave one question standing: whether an implicit assertion should invoke a throwing handler at all. Options E, F, and G answer it three ways - E removes the handler from implicit assertions, F gives them the response the committee already chose for `assert`, and G gives them the working draft's own ending - and this section presents each with the properties it provides and the cost it carries. It opens with those costs, in Section 6.1, because the case for these options is made by stating their limits at full weight and no more. The deployment record that bears on all three is held for Section 7 and applied there to every option alike.

### 6.1. What E, F, and G give up

Each option gives something up, and naming the loss precisely is the point of this subsection. Option E gives up the standard `observe` semantic for implicit assertions entirely: under E an implicit assertion cannot call the handler, so it cannot log a violation through the handler and continue. This is a real loss, and Section 6.2 states it in full. Sanitizer and tooling observation remains conforming under E - a build can still run UBSan or ASan and log - but that is tooling-based observation, not the contract handler, and the two are not the same facility. Option F gives up the escape path that Option A preserves, in exchange for the response `assert` already uses. Option G gives up a single hardwired ending, in exchange for the ending the working draft already defines.

### 6.2. Option E: no handler for implicit assertions
...
### 6.4. Option G: the working draft's own ending
...
The public poll record maps onto these options precisely, and the mapping cuts both ways. SG21 reached consensus for an optional opt-out that disallows the handler from throwing and otherwise calls `std::terminate` (Poll 7, 3/9/4/0/0).<sup>[7]</sup> The no-escape options - B and D among P3100's, and E, F, and G here - all belong to that opt-out's family, but the ending differs: Poll 7's stated ending is `std::terminate`, which matches Option B and the noexcept-enforce semantic, while F ends in `abort()` and G in contract-termination, so F and G realize the opt-out's intent with a different ending rather than being that poll. Against E specifically, the record cuts the other way: the broad restriction of implicit checks to non-handler-calling semantics - Option E's exact shape - was SG21's Poll 5, and it reached no consensus (2/3/4/6/3). That is a fact about E, stated here rather than left to a footnote. One scope note keeps the mapping exact: these polls were framed for the implicit checks that Profiles would add, while P3100 applies the same response question to all core-language undefined behavior, a broader set than the polls named.

---

## 7. Reading the Comparison

This section reads the two tables from Section 3. It completes Table 2 - the deployment column, withheld in Section 3, is filled in here with every cell cited - then states what the requirements grid rests on and the five dimensions the grid does not show. Each dimension is a finding from the public record, applied to every option by the same standard. The section reports these dimensions and states no ranking.

### 7.1. The deployment record

The rows of Table 2 are unchanged from Section 3 and remain in section order; only the deployment column is now filled. No option is implemented as a P3100 implicit-assertion feature, because P3100 itself is unimplemented, so the column reports whether each option's response shape - throw, terminate, trap, or abort - has field deployment, through the option where one exists and through an analogue otherwise, and it applies that standard to every row. The vocabulary is: *none* (the response shape ships nowhere), *analogues only* (a related mechanism ships, but not this response for this case), *prototype* (one experimental or opt-in implementation), *universal* (standard-mandated or shipped everywhere), and *massive-fleet* (measured production deployment at scale).
...
| C - add noexcept semantics | throw or terminate (selectable) | prototype - D4298R0 is implemented in the P3850 branches of GCC and Clang<sup>[12]</sup> |
...
### 7.2. What the requirements grid rests on
...
Five dimensions bear on that choice and do not appear in the grid. Each is stated below as a finding from the record, applied to every option alike.

### 7.3. Five dimensions the grid does not show
