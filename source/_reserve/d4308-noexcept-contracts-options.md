...
The response space for a throwing implicit contract assertion is eight options, not the four before EWG.
...
The public poll record maps onto these options precisely, and the mapping cuts both ways. The strong consensus for Option A that Section 4.2 records was taken on P3541R1's two-option choice, Option A against Option B; Options E, F, and G were not before SG21 then, so that consensus ranks A above B and not above the wider space enumerated here. SG21 reached consensus for an optional opt-out that disallows the handler from throwing and otherwise calls `std::terminate` (Poll 7, 3/9/4/0/0).<sup>[7]</sup> SG21 also polled a configurable choice between throwing and non-throwing evaluation semantics (Poll 3, 0/2/3/8/6), which reached consensus against<sup>[7]</sup>; Option C's design, in which the user selects between observe and observe-noexcept, is the shape that poll addressed. The no-escape options - B and D among P3100's, and E, F, and G here - all belong to that opt-out's family, but the ending differs: Poll 7's stated ending is `std::terminate`, which matches Option B and the noexcept-enforce semantic, while F ends in `abort()` and G in contract-termination, so F and G realize the opt-out's intent with a different ending rather than being that poll. Against E specifically, the record cuts the other way: the broad restriction of implicit checks to non-handler-calling semantics - Option E's exact shape - was SG21's Poll 5, and it reached no consensus (2/3/4/6/3). That is a fact about E, stated here rather than left to a footnote. One scope note keeps the mapping exact: these polls were framed for the implicit checks that Profiles would add, while P3100 applies the same response question to all core-language undefined behavior, a broader set than the polls named.
