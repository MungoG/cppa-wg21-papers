...
Six papers: two rule systems operating at different levels, one predictive scoring model, a thirteen-term vocabulary of committee failure modes, and the case for a missing integer type. Five papers trace the jurisdictional boundary between WG21's internal practice, which carries no ISO procedural standing, and the SC-level decisions where National Body rights live. One makes the evidence case for `std::int128_t`.
...
The procedural foundation of WG21 rests on two documents that produce opposite institutional outcomes - and only one of them was ever voted on by the committee. P4201R0 applies game-theoretic reasoning, six institutional forces from published research spanning 1911 to 2003, and five diagnostic tests from Great Founder Theory to the ISO Directives and SD-4 side by side, building on the jurisdictional analysis in P4240R0, which asks which rules governed the P2900 comment phase. P4201R0 maps the dominant strategy each system creates for eight distinct player classes and accompanies every finding with an observability statement tied to the committee's published record, delivering a structural explanation for why rational participants acting in good faith under SD-4 can produce outcomes the ISO Directives were specifically designed to prevent.
...
Any single paper in this collection delivers a specific, usable artifact: a game-theoretic strategy map, a scoring rubric, a failure-mode checklist, a set of reputation scenarios, or a sequence of self-diagnostic questions. Any cluster delivers a compound understanding no individual paper achieves - P4201R0 explains why the system drifts; the trilogy explains how participants accelerate or resist that drift; the scoring model quantifies the result. The full collection delivers something qualitatively different: a unified diagnostic framework that connects institutional structure to social dynamics to individual accountability, grounded at every step in the committee's own published record. The adoption figures P4201R0 traces - 43% at C++17, 34% at C++20, 21% at C++23, 7% at C++26 - are not an argument but a measurement, and the collection provides the structural vocabulary to explain the trend line.

Two entry points serve two reader profiles. A committee member who reviews large proposals should begin with P4202R0, whose checklist, thirteen named dynamics, and detailed case study of the networking gap - complete with reflector posts, direction polls, and revision histories - provide immediately applicable diagnostic tools. A reader interested in the theoretical model that unifies the collection should begin with P4201R0, which applies the deepest analytical framework and whose observability statements tell the reader exactly what to look for in the published record to confirm or disconfirm every finding independently. National Body delegates preparing for a ballot should first read P4240R0, which asks which rules governed the P2900 comment phase, or P4238R0 for the contracts case study.

---

## 3. Individual Papers

### 3.1. P4200R0 - The Peerage
...
### 3.4. P4203R0 - Big Reputation Requires Big Responsibility
...
### 3.5. P4204R0 - Big Papers Require Big Humility
...
### 3.6. D0001R0 - The Case for std::int128_t

Every major 64-bit compiler ships `__int128_t`. One will not implement it without a standard. The standard does not have it. D0001R0 documents the evidence for standardizing `std::int128_t` and `std::uint128_t` as extended integer types in `<cstdint>`, covering why the type belongs in the standard, what alternatives were considered, and where the evidence lives. The companion paper D0000R0 contains the proposed wording.
...
[1] P4200R0 - "The Peerage" (Vinnie Falco, 2026).

[2] P4201R0 - "Two Systems, One Committee: A Game-Theoretical Analysis of ISO Governance vs. SD-4" (Vinnie Falco, 2026).

[3] P4202R0 - "Big Claims Require Big Evidence" (Vinnie Falco, 2026).

[4] P4203R0 - "Big Reputation Requires Big Responsibility" (Vinnie Falco, 2026).

[5] P4204R0 - "Big Papers Require Big Humility" (Vinnie Falco, 2026).

[6] D0001R0 - "The Case for std::int128_t" (Vinnie Falco, 2026).
